...
# =========================================================
# ORIGINAL PACKAGES + REQUIRED TIGERVNC TOOLS
# =========================================================
...
# =========================================================
# FIREFOX VNC STABILITY FIX
# =========================================================

RUN mkdir -p /usr/local/bin && \
    cat > /usr/local/bin/firefox-vnc <<'EOF'
#!/bin/sh
...
RUN chmod 755 /usr/local/bin/firefox-vnc

# =========================================================
# FIREFOX DESKTOP LAUNCHER
# =========================================================

RUN mkdir -p /usr/share/applications && \
    if [ -f /usr/share/applications/firefox.desktop ]; then \
        sed -i 's#^Exec=.*#Exec=/usr/local/bin/firefox-vnc %u#' \
...
RUN if [ -f /opt/telegram/telegram.svg ]; then \
        ln -sf /opt/telegram/telegram.svg \
        /usr/share/icons/telegram.svg; \
    fi
...
RUN mkdir -p /etc/vnc && \
    ...
    'if [ -f "$HOME/Pictures/kali-wallpaper.jpg" ]; then' \
    '    xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor0/image-path -s "$HOME/Pictures/kali-wallpaper.jpg" 2>/dev/null || true' \
...
RUN mkdir -p /usr/local/bin && \
    cat > /usr/local/bin/start-vnc.sh <<'SCRIPT'
#!/bin/bash
...
printf '%s\n' "$VNC_PASS" | \
    su - "$RDP_USER" -c 'vncpasswd -f' \
    > "$VNC_PASS_FILE"

chown "$RDP_USER:$RDP_USER" "$VNC_PASS_FILE"
...
# =========================================================
# FIREFOX USER ENVIRONMENT
# =========================================================

cat > "$USER_HOME/.firefox-vnc-env" <<'EOF'
export MOZ_WEBRENDER=0
export MOZ_ACCELERATED=0
export LIBGL_ALWAYS_SOFTWARE=1
export MOZ_DISABLE_RDD_SANDBOX=1
...
chown "$RDP_USER:$RDP_USER" \
    "$USER_HOME/.firefox-vnc-env"

chmod 644 "$USER_HOME/.firefox-vnc-env"
...
if [ -n "$WALLPAPER_URL" ] && \
   [ "$WALLPAPER_URL" != "PASTE_RAW_GITHUB_URL_HERE" ]; then
    ...
    curl -L \
        ...
        --show-error \
        "$WALLPAPER_URL" \
        -o "$USER_HOME/Pictures/kali-wallpaper.jpg" \
        || true
fi

if [ -f "$USER_HOME/Pictures/kali-wallpaper.jpg" ]; then
    chown "$RDP_USER:$RDP_USER" \
        "$USER_HOME/Pictures/kali-wallpaper.jpg"
fi
...
chown "$RDP_USER:$RDP_USER" \
    "$USER_HOME/.config/autostart/xfce-panel.desktop"

# =========================================================
# DISABLE XFCE NOTIFICATIONS
# =========================================================
...
echo "COLOR    : 16-bit"
echo "COMPOSITOR: OFF"
echo "FIREFOX  : installed + VNC optimized"
